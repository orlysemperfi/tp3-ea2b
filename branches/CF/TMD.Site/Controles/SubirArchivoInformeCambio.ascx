<%@ Control Language="C#" AutoEventWireup="true" CodeBehind="SubirArchivoInformeCambio.ascx.cs"
    Inherits="TMD.CF.Site.Controles.SubirArchivoInformeCambio" %>

        <p>
            Subir Archivo Informe Cambio</p>
        <p>
            Archivo</p>
        <p>
            <asp:FileUpload ID="fileUpArchivo" runat="server" />
        </p>
        <p>
            <asp:Button ID="btnGrabar" runat="server" Text="Grabar" ValidationGroup="GrabarValidationGroup"
                OnClientClick="javascript: return grabar();" OnClick="btnGrabar_Click" />
            <asp:Button ID="btnCancelar" runat="server" Text="Cancelar" OnClick="btnCancelar_Click" />&nbsp;</p>
        &nbsp;<asp:HiddenField ID="hidIdSolicitud" runat="server" />
        </p>

<script type="text/javascript" language="javascript">
    function grabar() {
        if (Page_ClientValidate('GrabarValidationGroup')) {
            Page_BlockSubmit = false;
            return confirm('Desea grabar los datos ingresados?');
        }
        Page_BlockSubmit = false;
        return false;
    }
</script>